...
layui.define("jquery", function(e) {
	...
	i.prototype.init = function(e) {
		...
		var r = this,
			i = r.options,
			n = a || i.nodes,
			tp = i.type == 'checkbox', cl;//
		var iv = function(e){ //
			e.each(function(n, e){
				o(e).children('.tree-checkbox-checked').length > 0 &&
					o(e).children('ul').children('li').children('.tree-checkbox').addClass('tree-checkbox-checked'), iv(o(e).children('ul').children('li'));
			})
		}
		layui.each(n, function(a, n) {
			var l = n.children && n.children.length > 0,
				...
				s = o(["<li " + (n.spread ? 'data-spread="' + n.spread + '"' : "") + ">", function() {
					...
					//
					return '<a href="' + (n.href || "javascript:;") + '" ' + (i.target && n.href ? 'target="' + i.target + '"' : "") + ">" + ('<i class="layui-icon layui-tree-' + (l ? "branch" : "leaf") + '">' + (l ? n.spread ? t.branch[1] : t.branch[0] : t.leaf) + "</i>") + ("<cite>" + (n.name || "未命名") + '</cite></a>'+ (i.type == 'checkbox'?'<div class="tree-checkbox'+(n.flag?' tree-checkbox-checked':'')+'" data-id="'+ n.id +'"><b class="layui-icon">&#xe605;</b></div>':''))
				}(), "</li>"].join(""));
			l && (s.append(c), r.tree(c, n.children)), e.append(s), "function" == typeof i.click && r.click(s, n), r.spread(s, n), i.drag && r.drag(s, n)
		})
		var cl = o(i.elem).children(); tp && cl.length == i.nodes.length && iv(cl);  //初始化复选框
	}, i.prototype.click = function(e, o) {
		var a = this,
		r = a.options,
		c = 'tree-checkbox',
		cd = 'tree-checkbox-checked',
		tp = r.type == 'checkbox';
	//遍历是否全选状态
	var tf = function(e){
		var es = e.siblings().add(e),
			us = es.find('.'+c),
			f = true,
			pv = e.parent('ul').prev('.'+c);
		us.each(function(n, e){
			f = f && o(e).hasClass(cd);
		})
		f?(!pv.hasClass(cd) && pv.addClass(cd)):pv.removeClass(cd);
			pv.parent('li').length > 0 ?tf(pv.parent('li')):0;
		}
		
		function lc(e){
			var u = o(e), us = u.next().find('.'+c).add(u);
			u.attr('class').indexOf(cd) != -1?(us.removeClass(cd)):(us.addClass(cd));
			tf(u.parent('li'));
		}
		function ac(e){
			var u = o(e).next();
			u.attr('class').indexOf(cd) != -1?u.parent('li').find('.'+c).removeClass(cd):u.parent('li').find('.'+c).addClass(cd);
			tf(u.parent('li'))
		}
		
		//
		e.children('.'+c).on('click', function(){
			tp && lc(this);
		})
		//
		e.children("a").on("click", function(e){
			tp && ac(this);
			layui.stope(e), r.click(m)
		});
